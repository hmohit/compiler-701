...
namespace {
  class liveVars : public FunctionPass {
    ...
    virtual bool runOnFunction(Function &F) {
      ...
      std::vector<llvm::Instruction*> removeList;
...
//	      std::cerr << "Working on sets for: " << findKey(map, instrIter) << "\n" ;
...
/*
      ...
      for(; i!= map.end(); ++i){
          llvm::Instruction *I = i->first;          	
	  std::cerr << "working on instr number " << findKey(map, I) << "\n"; 
          int code = I->getOpcode();
          if(I->isCast() || I->isBinaryOp() || I->isShift() || code == 53 || code == 56 || code == 45 || code == 46 || code == 49 || code == 29 || code == 27 || code == 26){
	      std::map<llvm::Instruction*, int>::iterator itr = instrAfterMap.find(I);
//		std::cerr << "Number " << itr->second;
//              if(instrAfter[itr->second].find(I) == instrAfter[itr->second].end()){}
//	          std::cerr << "Remove " << findKey(map, I) << "\n";
          }
//	  std::cerr << "\n";
     }
*/
      ...
      for(; rItr != removeList.end(); ++rItr){
          std::cerr << "Removing " << findKey(map, *rItr) << "\n";
      }
