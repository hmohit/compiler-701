...
namespace {
  ...
  private:
    ...
    static const bool DEBUG_AVAIL = false;
    ...
    map<int, int> vregClassSpillCount;
...
//		usedRegClasses.insert(vregClass);
    ...
    void getUsedClasses(MachineFunction &Fn){
        for (MachineFunction::iterator MFIt = Fn.begin(), MFendIt = Fn.end(); MFIt != MFendIt; MFIt++) {
	    for (MachineBasicBlock::iterator MBBIt = MFIt->begin(), MBBendIt = MFIt->end(); MBBIt != MBBendIt; MBBIt++) {
	        //*MBBIt is a MachineInstr
	        int numOp = MBBIt->getNumOperands();
	        for (int i = 0; i < numOp; i++) {
	    	    MachineOperand &MOp = MBBIt->getOperand(i);  
			    ...
			    int vregClass = MRI->getRegClass(reg)->getID();
			    usedRegClasses.insert(vregClass);
			    std::set<unsigned> *spillSet = new std::set<unsigned>();
//			    regClassToSpillPregSetMap.insert(std::pair<int, std::set<unsigned>*>(vregClass, spillSet));
			    regClassToSpillPregSetMap[vregClass] = spillSet;
			    vregClassSpillCount[vregClass] = 0;
			}
		    }
	        }
	    }
...
//		std::cerr << "Working on instruction " << InstrToNumMap[BB] << "\n";
...
//				std::cerr << "Got frame index\n";
  				int vregClass = MRI->getRegClass(pReg)->getID();
//				std::cerr << "Class for register " << pReg << " is " << vregClass << "\n";
//				std::cerr << "Reg class?\n";
	...
	for (MachineFunction::iterator MF = Fn.begin(), MFendIt = Fn.end(); MF != MFendIt; MF++) {
		...
		//replace vreg with color
			...
			reg = MOp.getReg();	
			    ...
			    //virtual reg: replace with color
//			    pReg = regToColorMap[reg];
//			    std::cerr << "subRegs: virtual reg: " << reg << "\n";
			    pReg = regToColorMap[reg];
...
//    map<int, int> vregClassSpillCount;
	...
	return regClassToAvailPregSetMap[a]->size() < regClassToAvailPregSetMap[b]->size();
    }


    void checkForAliases(const TargetRegisterInfo *TRI, unsigned reg){
	std::set<int>::iterator i = usedRegClasses.begin();
        for(; i != usedRegClasses.end(); ++i){
	    if(*i != reg && vregClassSpillCount[*i] < 3){
		std::set<unsigned>::iterator alias = spillRegistersAll.begin();
		for(; alias != spillRegistersAll.end(); ++alias){
		    unsigned RUC = *alias;
	   	    if(checkForSpill(RUC, *i)){
	                //can be used
		        spillRegisters.insert(RUC); 
		        //add it's aliases to the other set
		        std::set<unsigned> *aliasOfSpills = new std::set<unsigned>();
	     	        addAliases(aliasOfSpills, RUC, TRI); 
		        aliasOfSpills->insert(RUC);   
		        spillRegistersAll.insert(aliasOfSpills->begin(), aliasOfSpills->end());
		        //update the list for this class
		        regClassToSpillPregSetMap[*i]->insert(RUC);
			//increment it's count
		        vregClassSpillCount[*i]++;
			//check if aliases are useful
		        break;
	            }
		}
	    }
...
//	std::map<int, std::set<unsigned>*>::iterator i = regClassToAvailPregSetMap.begin();	
	//store the classes in increasing order of available registers
//	std::vector<int> *order = new std::vector<int>(usedRegClasses.begin(), usedRegClasses.end());
//	std::vector<int> sorted = new std::vector<int>();
//	std::vector<int>::iterator m = order->begin();
//	for(; m != order->end(); ++m){
/*
	while(!order->empty()){
	    int min = 0;
	    int minReg = 0;
	    for(; m != order->end(); ++m){
		if(regClassToAvailPregSetMap[*m]->size() < min){
		    min = 
	        }
	    }
	}
*/
	for(int count = 0; count<3; count++){
	    ...
	    for(; i != usedRegClasses.end(); ++i){
		int vregClass = *i;
		if(vregClassSpillCount[vregClass] < 3){
        	    std::set<unsigned> *avail = regClassToAvailPregSetMap[vregClass];
        	    std::set<unsigned>::iterator aR = avail->begin();
		    //pick for the first register
        	    for(; aR != avail->end(); ++aR){
            	        unsigned RUC = *aR;
	    	        if(checkForSpill(RUC, vregClass)){
	                    //can be used
		            spillRegisters.insert(RUC); 
		            //add it's aliases to the other set
		            std::set<unsigned> *aliasOfSpills = new std::set<unsigned>();
		            ...
		            spillRegistersAll.insert(aliasOfSpills->begin(), aliasOfSpills->end());
			    //update the list for this class
		            regClassToSpillPregSetMap[vregClass]->insert(RUC);
			    //increment it's count
		            vregClassSpillCount[vregClass]++;
			    //check if aliases are useful
			    if(count != 0)
			        checkForAliases(TRI, vregClass);
		            break;
	                }
	            }
		}
...
//	    std::cerr << "Spill for class " << clas << " with count " << vregClassSpillCount[clas] << ": ";
