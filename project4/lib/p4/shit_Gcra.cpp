...
namespace {
  ...
  private:
    ...
    std::multimap<unsigned, std::set<unsigned>*> interferenceGraph;	//interference graph

    std::multimap<unsigned, std::vector<MachineInstr *>*> initRange;	//initial live range (reg -> instr)
    std::multimap<unsigned, std::vector<MachineInstr *>*> finalRange;	//initial live range (reg -> instr)
...
*/
	...
	std::multimap<unsigned, std::set<unsigned>*>::iterator regs = interferenceGraph.begin();
...
//	        (i->second)--;
    ...
    int unstackedNeighbors(unsigned reg, std::deque<unsigned> *regStack){
        int numNeighbors = 0;
	std::map<unsigned, std::set<unsigned>*>::iterator j = interferenceGraph.find(reg);
	if(j != interferenceGraph.end()){
	    std::set<unsigned> *neigh = j->second;
...
//	    printReg(reg->first);
	    if (TargetRegisterInfo::isVirtualRegister(reg->first)){
	    	...
	    	int numAvail = regClassToAvailPregSetMap[vregClass]->size();
	    	map->insert(std::pair<int, int>(vregClass, numAvail));
//		int temo = map[vregClass];
//		std::cerr << "Number of registers available for " << vregClass << " is " << map[vregClass] << "\n";
	    }	    
	}
    }
...
//     map<int, std::set<unsigned>*> regClassToAvailPregSetMap;
	...
	for(std::map<unsigned, std::vector<MachineInstr*>*>::iterator iter = finalRange.begin(); iter != finalRange.end(); ++iter){
	    std::vector<MachineInstr*> *t1 = iter->second;
	    unsigned r = iter->first;
	    std::set<MachineInstr*> *temp = new std::set<MachineInstr*>(t1->begin(), t1->end());
	    finalLiveRange.insert(std::pair<unsigned, std::set<MachineInstr*>*>(r, temp));
	}
...
//	std::cerr << "is : ";
//	printRegSet(graph);
	interferenceGraph.insert(std::pair<unsigned, std::set<unsigned>*>(reg, graph));
	}
    }
...
//	    }
	...
	std::cerr << "\nPhysical Registers\n";
    	std::multimap<unsigned, std::set<unsigned>*>::iterator i = interferenceGraph.begin(); 
	...
	std::multimap<unsigned, std::vector<MachineInstr *>*>::iterator itRange = initRange.begin();
	while(itRange != initRange.end()){
	    unsigned reg = itRange->first;
	    std::vector<MachineInstr *> *list = itRange->second;	//list under consideration
            std::vector<MachineInstr *> *combined = new std::vector<MachineInstr *>(list->begin(), list->end());
//	    combined->push_back(list->begin(), list->end());
	    std::multimap<unsigned, std::vector<MachineInstr *>*>::iterator i = itRange;
	    ++i;
	    for(; i != initRange.end(); ++i){
		std::vector<MachineInstr *> *nList = i->second;	//next list
		if(i->first == reg){
		    for(std::vector<MachineInstr *>::iterator j=nList->begin(); j != nList->end(); ++j){
		        if(std::find(combined->begin(), combined->end(), *j) != combined->end()){
//			    std::copy(nList->begin(), nList->end(), std::back_inserter(combined));
			    combined->insert(combined->begin(), nList->begin(), nList->end());
			    itRange++;
			    break;
		        }
		    }
	    ...
	    finalRange.insert(std::pair<unsigned, std::vector<MachineInstr *>*>(reg, combined));
	    itRange++;
	}
    }
	...
	for(std::set<unsigned>::iterator lIt = s->begin(); lIt != s->end(); lIt++){
            std::vector<MachineInstr*> *regRange = new std::vector<MachineInstr*>();	//store reg live range
	    unsigned reg = *lIt;
	    regRange->push_back(instr);	    
	    addToRange(regRange, reg, Fn, instr);
	    unsigned *regPtr = &reg;
	    initRange.insert(std::pair<unsigned, std::vector<MachineInstr*>*>(reg, regRange));
	}	
    }
    ...
    void addToRange(std::vector<MachineInstr*> *range, unsigned reg, MachineFunction &Fn, MachineInstr *I){
        for (MachineFunction::iterator bb = Fn.begin(), bbEnd = Fn.end(); bb != bbEnd; bb++) {
	    int firstInstr = 0;
	    for (MachineBasicBlock::iterator instr = bb->begin(); instr != bb->end(); instr++) {
		MachineBasicBlock *Basic = bb;
    ...
    void addToRangeFirst(std::vector<MachineInstr*> *range, unsigned reg, MachineFunction &Fn, MachineInstr *I){
        for (MachineFunction::iterator bb = Fn.begin(), bbEnd = Fn.end(); bb != bbEnd; bb++) {
	    int firstInstr = 0;
	    for (MachineBasicBlock::iterator instr = bb->begin(); instr != bb->end(); instr++) {
		MachineBasicBlock *Basic = bb;
	...
	std::cerr << "\nPhysical Registers\n";
	std::multimap<unsigned, std::vector<MachineInstr*>*>::iterator i = initRange.begin();	
	for(; i != initRange.end(); i++){
	    std::vector<MachineInstr*> *S = i->second;
	    std::vector<unsigned> temp;
	    for(std::vector<MachineInstr*>::iterator j=S->begin(); j!= S->end(); ++j){
	...
	std::cerr << "\nPhysical Registers\n";
	std::multimap<unsigned, std::vector<MachineInstr*>*>::iterator i = finalRange.begin();	
	for(; i != finalRange.end(); i++){
	    std::vector<MachineInstr*> *S = i->second;
	    std::vector<unsigned> temp;
	    for(std::vector<MachineInstr*>::iterator j=S->begin(); j!= S->end(); ++j){
		unsigned no = InstrToNumMap[*j];
